...
impl<T: Clone> Tensor<T> {
    pub fn view(&'_ self) -> TensorView<'_, T> {
        TensorView {
            data: &self.get_data(),
            shape: (&self.get_shape()).to_vec(),
            strides: self.get_strides().to_vec(),
            offset: *self.get_offset(),
        }
    }

    pub fn view_mut(&'_ mut self) -> TensorView<'_, T> {
        TensorView {
            data: &mut self.get_data(),
            shape: self.get_shape().to_vec(),
            strides: self.get_strides().to_vec(),
            offset: *self.get_offset(),
        }
